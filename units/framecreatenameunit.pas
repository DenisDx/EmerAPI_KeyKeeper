...
uses
  ...
  Spin, GUIHelperUnit, fpjson, EmerAPIBlockchainUnit, CryptoLib4PascalConnectorUnit;
...
begin
  ...
  nValue:=trim(clear13(meValue.text));
 ...
 if sender.id=('checkname:'+trim(eName.Text)) then begin
  ...
  if e<>nil then
     if e.IsNull then begin
       lNameExits.Caption:=localizzzeString('FrameCreateName.NameIsFree','Name is not registered yet');
